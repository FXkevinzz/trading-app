...
# --- GESTIÓN DE DATOS (JOURNAL - NUEVO) ---
...
# --- ESTILOS CSS (TU ESTILO ORIGINAL) ---
st.markdown("""
    <style>
    /* Estilos Generales */
    ...
    .stTabs [data-baseweb="tab"] { height: 50px; white-space: pre-wrap; background-color: #161B22; border-radius: 4px 4px 0px 0px; gap: 1px; padding-top: 10px; padding-bottom: 10px; color: white;}
    .stTabs [aria-selected="true"] { background-color: #1f7a1f !important; color: white !important; }
    </style>
...
with st.sidebar:
    ...
    if api_key:
        try:
            ...
            # Mostrar solo los últimos 2 mensajes para no llenar la barra
...
tab1, tab2, tab3 = st.tabs(["🦁 CHECKLIST OPERATIVO", "📝 REGISTRAR TRADE", "📊 DASHBOARD & HISTORIAL"])
...
# ==========================================
# TAB 2: REGISTRO DE TRADES (NUEVO)
# ==========================================
with tab2:
    ...
    with st.form("trade_form"):
        ...
        with c3:
            ratio_reg = st.number_input("Ratio RR (Ej: 2.5)", min_value=0.0, step=0.1, value=2.5)
            notas_reg = st.text_area("Notas / Emociones", placeholder="Ej: Me sentí ansioso, entré tarde...")
...
# ==========================================
# TAB 3: DASHBOARD & HISTORIAL (NUEVO)
# ==========================================
with tab3:
    ...
    if not df.empty:
        ...
        def color_result(val):
            if 'WIN' in str(val): return 'background-color: #1f7a1f; color: white;'
            if 'LOSS' in str(val): return 'background-color: #5c0000; color: white;'
            return 'color: white;'
        ...
        st.download_button("📥 Descargar Backup", csv, "journal_backup.csv", "text/csv")
        
    else:
        st.info("Aún no hay trades registrados. Usa la pestaña 'Registrar Trade'.")
