...
def save_image_locally(image_obj, filename):
    """Guarda la imagen física en el servidor."""
    try:
...
def save_to_brain(analysis_text, pair, result, mode, images_list=None):
    """Guarda el análisis en el cerebro compartido."""
    memory = load_brain()
...
def analyze_multiframe(images_data, mode, pair):
    """Análisis de 3 imágenes para validar Sincronía."""
    brain = load_brain()
    context = ""
    if brain:
...
def generate_audit_report(df):
    ...
    2. Zonas de Poder (Mejores setups).
...
# ==========================================
# 4. SISTEMA DE TEMAS (CSS EXPANDIDO)
# ==========================================
...
def delete_trade(u, acc, index):
    """Borra un trade específico del CSV."""
...
def main_app():
    ...
    with tabs[0]:
        ...
        if "Swing" in modo:
            ...
            with r1_c1:
                ...
                w_sc = sum([st.checkbox("AOI (+5%)", key="sc1")*5, st.checkbox("Rechazo Estructura (+5%)", key="sc2")*5, st.checkbox("Patrón (+5%)", key="sc3")*5, st.checkbox("EMA 50 (+5%)", key="sc4")*5, st.checkbox("Psico (+5%)", key="sc5")*5])
                st.markdown('</div>', unsafe_allow_html=True)
            with r1_c2:
                st.markdown('<div class="strategy-box">', unsafe_allow_html=True)
    ...
    with tabs[1]:
        ...
        with sub_ia[0]:
            st.markdown(f"<h3 style='color:var(--accent)'>🧠 Análisis Multi-Imagen</h3>", unsafe_allow_html=True)
    ...
    # --- TAB 3: BITÁCORA (CON BOTÓN BORRAR) ---
    with tabs[2]:
