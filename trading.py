...
# ==============================================================================
# 1. CONFIGURACIÓN DE LA PÁGINA (OBLIGATORIO AL INICIO)
# ==============================================================================
st.set_page_config(
...
def inject_theme(theme_mode):
    ...
    .stTabs [data-baseweb="tab"] {{ background-color: var(--bg-card) !important; color: var(--text-muted) !important; border: 1px solid var(--border-color); border-radius: 50px !important; padding: 0 25px !important; height: 45px; box-shadow: var(--shadow); font-weight: 600; }}
    .stTabs [data-baseweb="tab"][aria-selected="true"] {{ background-color: var(--accent) !important; color: white !important; border: none !important; box-shadow: 0 4px 10px rgba(0,0,0,0.2); }}
    ...
    with tabs[0]:
        ...
        if st.session_state.global_mode == "Swing (W-D-4H)":
            ...
            with r2_c2:
                ...
                sos = st.checkbox("⚡ Shift of Structure (SOS)"); eng = st.checkbox("🕯️ Vela Envolvente"); pat_ent = st.checkbox("Patrón Entrada (+5%)"); rr = st.checkbox("💰 Ratio > 1:2.5")
