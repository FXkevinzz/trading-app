...
# ==============================================================================
# 4. LÓGICA DE HORARIO DE MERCADO (PDF)
# ==============================================================================
...
def main_app():
    ...
    # --- TICKER (BARRA DE PRECIOS RESTAURADA) ---
    tv_theme = "dark"
    ...
    # --- TAB 1: OPERATIVA (CHECKLIST COMPLETO) ---
    with tabs[0]:
        ...
        if "Swing" in modo:
        ...
        else: # SCALPING (LÓGICA DEL PDF)
            with r1_c1:
                st.markdown('<div class="strategy-box">', unsafe_allow_html=True)
                st.markdown(header("1. CONTEXTO (4H)"), unsafe_allow_html=True)
                t4 = st.selectbox("Trend 4H", ["Alcista", "Bajista"], key="s4")
                w_sc = sum([
                    st.checkbox("Rechazo AOI (+5%)", key="sc1")*5,
                    st.checkbox("Rechazo Estructura (+5%)", key="sc2")*5,
                    st.checkbox("Patrón Vela (+5%)", key="sc3")*5,
                    st.checkbox("EMA 50 (+5%)", key="sc4")*5,
            ...
            with r1_c2:
                ...
                d_sc = sum([
                    st.checkbox("Rechazo AOI (+5%)", key="s21")*5,
                    st.checkbox("Rechazo Estructura (+5%)", key="s22")*5,
                    st.checkbox("Vela Rechazo (+5%)", key="s23")*5,
                    st.checkbox("Patrón (+5%)", key="s24")*5,
            ...
            with r2_c1:
                ...
                h4_sc = sum([
                    st.checkbox("Vela Rechazo (+5%)", key="s31")*5,
                    st.checkbox("Patrón (+5%)", key="s32")*5,
                    st.checkbox("Rechazo Estructura (+5%)", key="s33")*5,
                    st.checkbox("EMA 50 (+5%)", key="s34")*5
                ])
                st.markdown('</div>', unsafe_allow_html=True)
            ...
            with r2_c2:
                ...
                eng = st.checkbox("🕯️ Vela Entrada (Obligatorio)")
                pat_ent = st.checkbox("Patrón Entrada (+5%)")
                rr = st.checkbox("💰 Ratio Mínimo 1:2.5")
                
                entry_score = sum([sos*10, eng*10, pat_ent*5])
                total = w_sc + d_sc + h4_sc + entry_score + 15 # 15 pts base por Scalp
        ...
        <div class="hud-container">
            <div class="hud-stat"><div class="hud-label">PUNTAJE TOTAL</div><div class="hud-value-large">{total}%</div></div>
    ...
    with tabs[1]:
                ...
                with c_img:
                    ...
                    if st.button("🦁 ANALIZAR SINCRONÍA", type="primary", use_container_width=True):
                        ...
                        if not images_data: st.warning("Sube imágenes.")
                            ...
                            with st.spinner("Analizando Sincronía..."):
        ...
        with sub_ia[1]:
            ...
            else: st.info("Sin trades ganadores con imagen.")
