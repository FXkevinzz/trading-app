...
with st.sidebar:
    ...
    if api_key:
        try:
            ...
            # USAMOS EL MODELO FLASH (Requiere librería actualizada)
            ...
            if prompt := st.chat_input("Consulta..."):
                ...
                prompt_sys = f"Eres experto en Set&Forget ({contexto}). Reglas: SL 5-7 pips fuera AOI, No BE. Usuario: {prompt}"
                
                resp = model.generate_content(prompt_sys)
    ...
    notas = st.text_area("Plan", height=150)
...
with col_checklist:
    if "Swing" in modo:
        ...
        with c1:
            ...
            w_score = sum([st.checkbox("En/Rechazo AOI (+10%)", key="w1")*10, st.checkbox("Rechazo Estruc. Previa (+10%)", key="w2")*10, st.checkbox("Patrones (+10%)", key="w3")*10, st.checkbox("Rechazo EMA 50 (+5%)", key="w4")*5, st.checkbox("Nivel Psicológico (+5%)", key="w5")*5])
        with c2:
            st.subheader("2. Diario (D)")
            d_score = sum([st.checkbox("En/Rechazo AOI (+10%)", key="d1")*10, st.checkbox("Rechazo Estruc. Previa (+10%)", key="d2")*10, st.checkbox("Rechazo Vela (+10%)", key="d3")*10, st.checkbox("Patrones (+10%)", key="d4")*10, st.checkbox("Rechazo EMA 50 (+5%)", key="d5")*5])
        st.divider()
        c3, c4 = st.columns(2)
        with c3:
            st.subheader("3. Ejecución (4H)")
            h4_score = sum([st.checkbox("Rechazo Vela (+10%)", key="h1")*10, st.checkbox("Patrones (+10%)", key="h2")*10, st.checkbox("En/Rechazo AOI (+5%)", key="h3")*5, st.checkbox("Rechazo Estructura (+5%)", key="h4")*5, st.checkbox("Rechazo EMA 50 (+5%)", key="h5")*5])
        with c4:
        ...
        with c1:
            ...
            w_score = sum([st.checkbox("En/Rechazo AOI (+5%)", key="sc1")*5, st.checkbox("Rechazo Estruc. Previa (+5%)", key="sc2")*5, st.checkbox("Patrones (+5%)", key="sc3")*5, st.checkbox("Rechazo EMA 50 (+5%)", key="sc4")*5, st.checkbox("Nivel Psicológico (+5%)", key="sc5")*5])
        with c2:
            st.subheader("2. Contexto 2H")
            d_score = sum([st.checkbox("En/Rechazo AOI (+5%)", key="sc6")*5, st.checkbox("Rechazo Estruc. Previa (+5%)", key="sc7")*5, st.checkbox("Rechazo Vela (+5%)", key="sc8")*5, st.checkbox("Patrones (+5%)", key="sc9")*5, st.checkbox("Rechazo EMA 50 (+5%)", key="sc10")*5])
        st.divider()
        c3, c4 = st.columns(2)
        with c3:
            st.subheader("3. Ejecución (1H)")
            h4_score = sum([st.checkbox("Rechazo Vela (+5%)", key="sc11")*5, st.checkbox("Patrones (+5%)", key="sc12")*5, st.checkbox("Rechazo Estruc. Previa (+5%)", key="sc13")*5, st.checkbox("Rechazo EMA 50 (+5%)", key="sc14")*5])
        with c4:
...
with col_resultados:
    ...
    if is_valid and total >= 60:
        st.success("### ✅ EJECUTAR")
        st.balloons()
        plan = "<b>Plan Scalping:</b><br>• Entrada: M15/30<br>• SL: 3-5 pips<br>• TP: Estructura" if "Scalp" in modo else "<b>Plan Swing:</b><br>• Entrada: 4H<br>• SL: 5-7 pips<br>• TP: Estructura"
